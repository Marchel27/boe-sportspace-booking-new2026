    ...
    <main class="flex-1 md:ml-64 p-6 md:p-10">
        ...
        <section>
            <div class="flex items-center justify-between mb-8">
                <div>
                    <h3 class="text-lg font-bold text-slate-700">Daftar Fasilitas</h3>
                    <p class="text-sm text-slate-500">Total {{ $lapangan->count() }} Fasilitas tersedia</p>
                </div>
                ...
                <div class="group bg-white rounded-[2rem] overflow-hidden border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-blue-100 transition-all duration-500 hover:-translate-y-2">
                    ...
                    <div class="p-6">
                        ...
                        <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-50">
                            <button onclick="confirmDelete({{ $item->id_lap }}, this)"
                                class="p-3 rounded-xl bg-red-50 text-red-500 hover:bg-red-500 hover:text-white transition-all duration-300 active:scale-90"
                                title="Hapus">
                            ...
                            <a href="{{ route('lapangan.edit', $item->id_lap) }}"
                                class="flex items-center gap-2 px-5 py-3 rounded-xl border border-slate-200 text-slate-600 hover:border-[#1265A8] hover:text-[#1265A8] transition-all duration-300 active:scale-90 font-medium text-sm"
                                title="Edit">
        ...
        function confirmDelete(id, button) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch(`/admin/dashboard/lapangan/${id}`, {
                            method: 'DELETE',
                            headers: {
